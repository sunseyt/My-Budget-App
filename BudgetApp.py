...
def debit():
    ...
    try:
        wRequest = int(input('Press (1) to continue with your debit transaction and (2) to stop debit transaction\n'))
    except:
        print('\nInvalid option')
        wRequest = int(input(
            'Press (1) to continue with your debit transaction and (2) to stop debit transaction\n'))
    if wRequest == 1:
        userRequest = input('Please select the budget account you would like to withdraw from, from the list given above \n')
        if userRequest in myDatabase:
            ...
            wAmount = int(input('Enter the amount you would like to withdraw: \n'))
            if wAmount < myDatabase[userRequest]:
                ...
                print(f'#{wAmount} has been debited from budget {userRequest}\n Budget balance is {newBalance}')
                menu()
            else:
                ...
                if wAmount == 1:
                    dAmount = int(input('Enter the amount you would like ti deposit: \n#'))
                    balance = int(myDatabase[userRequest])
                    newBalance =Budget.deposit(dAmount, balance)
                    myDatabase[userRequest] = newBalance
                    print(' ')
                    print(f'Budget {userRequest} has been credited with #{dAmount}\n')
                    debit()
                elif wAmount == 2:
                    debit()
                else:
...
def credit():
    ...
    if dOption == 1:
        ...
        if userOption in myDatabase:
            ...
            print(' ')
            userOption = int(input(f'Budget {userOption} does not exist!\nPress (1) to create a new budget\nPress (2) to choose the right budget\nPress (3) to perform another transaction'))
            if userOption == 1:
                newBudget
            elif userOption == 2:
                credit()
...
def transfer():
    ...
    if fundingBudget in myDatabase:
        try:
            fundingAmt = int(input('Enter the amount you want to transfer: \n'))
        except:
            print('\nInvalid input') #Just corrected this place
            fundingAmt = int(input('Enter the amount you want to transfer: \n'))
        if fundingAmt < myDatabase[fundingBudget]:
            receivingBudget = input('Enter the budget you want to transfer to:\n')
            if receivingBudget in myDatabase:
                db = Budget.transfer(myDatabase, fundingBudget, fundingAmt, receivingBudget)
                print('')
                print(f'You have successfully transferred #{fundingAmt} from {fundingBudget} to {receivingBudget}')
            ...
            print(f'You do not have such amount #{fundingAmt} in {fundingBudget}')
            transfer()
    else:
...
def out():
    userChoice = int(input('Are you sure you want to quit?: \nPress (1) to quit\nPress (2) to continue with other transactions'))
    if userChoice == 1:
